...
export const CloudSyncModal: React.FC<CloudSyncModalProps> = ({
  ...
  const handleMigrate = async () => {
    ...
    if (!currentUser) {
      setAuthError('Please sign in with Google first so Firestore security rules grant write access to your business records.');
      return;
    }
          ...
          <div className="p-3.5 rounded-xl bg-white border border-[#E3DDD1] space-y-2.5">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                <Lock className="w-5 h-5 text-[#45634D] flex-shrink-0 mt-0.5" />
                <div className="space-y-0.5">
                  <h4 className="font-bold text-[#1C211E] text-xs">Firebase Authentication & Security Rules</h4>
                  <p className="text-[11px] text-[#55635B] leading-relaxed">
                    Security rules defined in <code className="bg-[#F0EBE1] px-1 py-0.5 rounded font-mono text-[10px]">firestore.rules</code> enforce that only authenticated business operators can read or write business records.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="pt-2 border-t border-[#F0EBE1] flex items-center justify-between gap-2">
                {currentUser ? (
                  <div className="flex items-center gap-2 text-xs">
                    <UserCheck className="w-4 h-4 text-[#3A6B48]" />
                    <span className="text-[#23432B] font-medium">
                      Signed in as <strong className="font-semibold">{currentUser.email || 'Business User'}</strong>
                    </span>
                  </div>
                ) : (
                  <div className="text-[11px] text-[#9A3412] flex items-center gap-1.5 font-medium">
                    <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0" />
                    <span>Sign in required to authorize Firestore write operations.</span>
                  </div>
                )}
                ...
                <div>
                  {currentUser ? (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-[11px] font-semibold text-[#697A6F] hover:text-[#1C211E] hover:bg-[#F0EBE1] border border-[#D5DDD7] transition cursor-pointer flex items-center gap-1"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-bold bg-[#45634D] hover:bg-[#38523F] text-white transition cursor-pointer flex items-center gap-1.5 shadow-xs disabled:opacity-50"
                    >
                      {isSigningIn ? (
                        <>
